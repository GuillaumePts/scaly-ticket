...
def render_full_band(data_list):
    # La largeur MUST be a perfect multiple of 8 !!!
    # 800 dots = 100 bytes exacts !
    main = Image.new('1', (800, 1200), color=1)
...
if __name__ == '__main__':
    ...
    img.save("test34_alignment_preview.png")
    job = build_job_aligned(img)
    if '--print' in sys.argv:
        try:
            ...
            time.sleep(2.0)
            s.close()
            print("Envoyé (W=800 aligned)")
        except Exception as e:
            print(f"Erreur: {e}")
    else:
        print("Pret. Preview generee.")
