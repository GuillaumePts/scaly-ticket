...
print("=== RECHERCHE DES ÉCRITURES AVEC NUMÉRO DE LOT (Lot_No ne '') ===")

# Filtrer les écritures qui ont un vrai numéro de lot
raw_url = f"https://api.businesscentral.dynamics.com/v2.0/{tenant_id}/{env}/ODataV4/Company('{comp_encoded}')/ItemLedgerEntries?$filter=Lot_No ne ''&$top=20"
...
try:
    ...
    with urllib.request.urlopen(req) as resp:
        ...
        print(f"\nÉcritures avec Lot trouvées ({len(entries)}) :")
        for e in entries:
            ...
            print(f"  * [{item_no}] {desc} | Lot: {lot} | DLC: {exp_date} | StockRestant: {rem_qty} | Date: {pdate} | Emplacement: {loc}")
